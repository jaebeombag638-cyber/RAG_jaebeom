...
class PracticeChecks(unittest.TestCase):
    ...
    def test_no_evidence_never_calls_llm(self):
        with patch.object(rag, "retrieve", return_value=("질문", [], {})), patch.object(rag, "OpenAI") as llm:
    ...
    def test_missing_key_never_calls_llm(self):
        with patch.object(rag, "retrieve", return_value=("질문", [{"text": "본문",
                ...
                patch.dict(rag.os.environ, {"OPENAI_API_KEY": ""}), patch.object(rag, "OpenAI") as llm:
            with self.assertRaises(ValueError):
                rag.answer("질문", True)
            llm.assert_not_called()
    ...
    def test_question_to_answer_mocked_services(self):
        ...
        with tempfile.TemporaryDirectory() as directory, \
                ...
                patch.object(rag, "OpenAI") as llm:
            llm.return_value.responses.create.return_value.output_text = "테스트카페0은 카페입니다. [1]"
